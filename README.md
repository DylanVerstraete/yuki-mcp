...
| Tool | Key parameters | Description |
|------|----------------|-------------|
| `upload_document` | `fileName`, `dataBase64`, `folder?`, `amount?` | Upload a PDF to the Yuki archive with optional financial metadata. Use `get_document_folders` first to find the right folder ID. |
...
└── tools/
    ...
    └── documents.ts        # upload_document, get_document_folders
